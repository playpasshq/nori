...
class Nori
  ...
  class XMLUtilityNode
    def initialize(options, name, attributes = {})
      ...
      @nil_element = false
      @attributes = if options[:convert_attributes_to]
                      intermediate = attributes.map { |k, v| options[:convert_attributes_to].call(k, v) }.flatten
                      clean_attributes(Hash[*intermediate])
                    else
